...
import { deployContract, validAddress } from "../contract/securesignContract";

const { Step } = Steps;

function CreateSecureSign(props) {
  ...
  const create = async () => {
    ...
    try {
      ...
      const metadataHash = await uploadFiles(
        files,
        data.title,
        data.description,
        data.signerAddress,
      ...
      res["signatureUrl"] = signatureUrl(metadataHash);
      res["hash"] = metadataHash;
...
CreateSecureSign.propTypes = {};

export default CreateSecureSign;
